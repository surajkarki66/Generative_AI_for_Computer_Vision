...
# Define the GAN components with feature extraction capability
...
class Discriminator(nn.Module):
    def __init__(self, input_dim=2, extract_features=False):
        super(Discriminator, self).__init__()
        self.extract_features = extract_features
        
        # Define layers with names for easy access
    ...
    def forward(self, x):
        # Extract features from the second-to-last layer for feature matching
        x1 = self.leaky_relu1(self.fc1(x))
        x1 = self.dropout1(x1)
        
        x2 = self.leaky_relu2(self.fc2(x1))
        x2 = self.dropout2(x2)
        
        x3 = self.leaky_relu3(self.fc3(x2))
        x3 = self.dropout3(x3)
        
        output = self.sigmoid(self.fc4(x3))
        
        if self.extract_features:
            # Return both the output and the features from the last hidden layer
            return output, x3
        else:
            return output
...
discriminator = Discriminator(input_dim=2, extract_features=True)
...
class FeatureMatchingLoss(nn.Module):
    def __init__(self, feature_layer=-1):
    ...
    def forward(self, real_features, fake_features):
        ...
        Returns:
            L2 loss between mean features
        """
        # Calculate mean features for real and fake data
        real_mean = torch.mean(real_features, dim=0)
        fake_mean = torch.mean(fake_features, dim=0)
        
        # L2 loss between the mean features
        loss = torch.norm(real_mean - fake_mean, p=2) ** 2
...
feature_matching_criterion = FeatureMatchingLoss()
...
g_adv_losses, g_feature_losses = [], []  # Store adversarial and feature matching losses separately

current_noise_std = NOISE_STD
print("\nStarting training with Feature Matching...")
for epoch in range(n_epochs):
    current_noise_std = max(MIN_NOISE_STD, current_noise_std * NOISE_DECAY)
    
    epoch_g_adv_loss = []
    epoch_g_feature_loss = []
    ...
    for batch_idx, (real_batch,) in enumerate(dataloader):
        ...
        real_output, real_features = discriminator(real_batch_noisy)
        ...
        fake_output, fake_features = discriminator(fake_data_noisy.detach())
        ...
        # Get discriminator outputs and features for generated data
        fake_output, fake_features_gen = discriminator(fake_data_noisy)
        
        # Adversarial loss (standard GAN loss)
        g_adv_loss = criterion_regular(fake_output, real_labels)
        
        # Get features for real data (detached, we don't want to backprop through discriminator for this)
        with torch.no_grad():
            real_batch_for_features = add_noise_to_samples(real_batch, current_noise_std)
            real_features_for_fm = discriminator.get_features(real_batch_for_features)
        
        # Feature matching loss
        g_feature_loss = feature_matching_criterion(real_features_for_fm, fake_features_gen)
        
        # Combined generator loss
        g_loss = g_adv_loss + FEATURE_MATCHING_WEIGHT * g_feature_loss
        ...
        if batch_idx == len(dataloader) - 1:
            ...
            epoch_g_adv_loss.append(g_adv_loss.item())
            epoch_g_feature_loss.append(g_feature_loss.item())
    
    # Store average adversarial and feature matching losses for this epoch
    if epoch_g_adv_loss:
        g_adv_losses.append(np.mean(epoch_g_adv_loss))
        g_feature_losses.append(np.mean(epoch_g_feature_loss))
    ...
    if epoch % 5 == 0:
        print(f"Epoch [{epoch:3d}/{n_epochs}] | "
              ...
              f"G Adv: {g_adv_losses[-1]:.4f} | G FM: {g_feature_losses[-1]:.4f} | "
              f"KL: {kl_divergences[-1]:.4f}")
...
metrics_df = pd.DataFrame({
    ...
    'generator_adv_loss': g_adv_losses,
    'generator_feature_loss': g_feature_losses,
...
axes[0, 1].plot(epochs_plotted, g_losses, label='Generator Total', linewidth=2, color='red', alpha=0.7)
axes[0, 1].plot(epochs_plotted, d_losses, label='Discriminator', linewidth=2, color='blue', alpha=0.7)
axes[0, 1].plot(epochs_plotted, g_adv_losses, label='G Adv Loss', linewidth=1.5, color='orange', alpha=0.7, linestyle='--')
axes[0, 1].plot(epochs_plotted, g_feature_losses, label='G Feature Loss', linewidth=1.5, color='green', alpha=0.7, linestyle='--')
axes[0, 1].set_xlabel('Epoch')
axes[0, 1].set_ylabel('Loss')
axes[0, 1].set_title('Loss Curves (with Feature Matching)')
axes[0, 1].legend()
...
# Plot 6: Noise decay and feature matching ratio
ax6 = axes[1, 2]
ax6.plot(epochs_plotted, noise_levels, label='Noise Level', linewidth=2, color='brown')
ax6.set_xlabel('Epoch')
ax6.set_ylabel('Noise σ')
ax6.set_title('Noise Decay')
ax6.legend()
ax6.grid(True, alpha=0.3)

# Add a second y-axis for feature matching ratio
ax6_secondary = ax6.twinx()
feature_ratio = np.array(g_feature_losses) / (np.array(g_adv_losses) + 1e-8)
ax6_secondary.plot(epochs_plotted, feature_ratio, label='Feature/Adv Ratio', linewidth=1.5, color='green', alpha=0.7, linestyle=':')
ax6_secondary.set_ylabel('Feature/Adv Loss Ratio', color='green')
ax6_secondary.tick_params(axis='y', labelcolor='green')
ax6_secondary.legend(loc='upper right')
...
config_stats = {
    ...
    "training_results": {
        ...
        "final_generator_adv_loss": float(g_adv_losses[-1]),
        "final_generator_feature_loss": float(g_feature_losses[-1]),
...
print("\n" + "="*60)
print("TRAINING SUMMARY (with Feature Matching)")
print("="*60)
print(f"\nTarget Distribution:")
...
print(f"\nTraining Results:")
print(f"  Final G Total Loss: {g_losses[-1]:.4f}")
print(f"  Final G Adv Loss: {g_adv_losses[-1]:.4f}")
print(f"  Final G Feature Loss: {g_feature_losses[-1]:.4f}")
...
print(f"  Feature Layer: Last hidden layer (32 dimensions)")
print(f"  Loss Type: L2 norm between mean features")
# Save model
